...
const BlogPostAIAssistant = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      // console.log(values.prompt, values.tone, values.category);
      const prompt = `Generate a blog post based ${values.prompt},${values.category},${values.tone}`;

      const generatedContent = await mutation.mutateAsync({ prompt });
      // console.log(generatedContent, 'contentprompt');
